...
const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: {
    ...
    body: 'Este link não corresponde a um convite válido. Verifique a URL ou peça para a pessoa que o convidou enviar um novo link.',
  },
  used: {
  ...
  expired: {
    ...
    body: 'Este convite expirou. Peça ao administrador da conta para enviar um novo — eles levam apenas alguns segundos para serem gerados.',
  },
  server_error: {
    title: 'Algo deu errado',
    body: 'Não conseguimos verificar este convite no momento. Tente atualizar a página em alguns instantes.',
  },
};
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              'Você já está em outra conta. Faça login com um e-mail diferente para entrar nesta.',
          );
        } else {
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              Ao aceitar, seu login será movido para{' '}
              <span className="text-muted-foreground">{peek.account_name}</span>. Sua
              conta pessoal vazia criada no cadastro será excluída.
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                ...
                O link de convite permanece válido desde que não tenha
                expirado.
              </p>
            </div>
