...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Certifique-se de que o CSV possui uma coluna com o cabeçalho "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      try {
        ...
        toast.warning('Contatos importados, mas algumas atribuições de tags falharam.');
      }

      setResult({ imported, skipped, failed, tagsAssigned });
      if (imported > 0) {
        toast.success(
          `${imported} ${imported !== 1 ? 'contatos importados' : 'contato importado'}`
        );
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} ${tagsAssigned !== 1 ? 'atribuições de tags aplicadas' : 'atribuição de tag aplicada'}`
        );
      }
      if (skippedNames.length > 0) {
        ...
        toast.info(
          `Tags desconhecidas ignoradas (crie-as nas Configurações primeiro): ${sample}${more}`
        );
      }
      if (skipped > 0) {
        toast.info(`${skipped} ${skipped !== 1 ? 'duplicados ignorados' : 'duplicado ignorado'}`);
      }
      if (failed > 0) {
        toast.error(
          `Falha ao importar ${failed} ${failed !== 1 ? 'contatos' : 'contato'}`
        );
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Faça o upload de um arquivo CSV contendo a coluna obrigatória{' '}
              ...
              (separadas por vírgula; use aspas para múltiplos valores).
            </DialogDescription>
          </DialogHeader>
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {parsedRows.length} {parsedRows.length === 1 ? 'linha pronta' : 'linhas prontas'}
                </span>
              </>
            ) : (
              <>
                ...
                <p className="text-[11px] text-muted-foreground">
                  .csv dentro do limite do seu navegador
                </p>
              </>
            )}
          </div>
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  Pré-visualização · {preview.length === 1 ? 'primeira linha' : `primeiras ${preview.length} linhas`}
                </p>
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-md bg-muted/90 px-2 py-0.5 text-[11px] text-muted-foreground">
                      ...
                      {tagStats.unique} {tagStats.unique === 1 ? 'tag' : 'tags'} ·{' '}
                      {tagStats.rowsWithTags} {tagStats.rowsWithTags === 1 ? 'contato' : 'contatos'}
                    </span>
                  )}
                </div>
              </div>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + {parsedRows.length - PREVIEW_LIMIT} {parsedRows.length - PREVIEW_LIMIT === 1 ? 'outra linha' : 'outras linhas'} não exibidas
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {result.imported > 0 && (
                  <div className="text-primary flex items-center gap-1.5 text-sm">
                    ...
                    {result.imported} {result.imported === 1 ? 'importado' : 'importados'}
                  </div>
                )}
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    <CheckCircle className="size-4 shrink-0" />
                    {result.tagsAssigned} {result.tagsAssigned === 1 ? 'tag atribuída' : 'tags atribuídas'}
                  </div>
                )}
                {result.skipped > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-amber-400">
                    <AlertTriangle className="size-4 shrink-0" />
                    {result.skipped} {result.skipped === 1 ? 'ignorado' : 'ignorados'}
                  </div>
                )}
                {result.failed > 0 && (
        ...
        <DialogFooter className="mt-0 shrink-0 gap-2 border-t border-border/80 bg-background/50 px-6 py-4 sm:justify-end">
          ...
          {!result && (
            ...
            >
              ...
              Importar {parsedRows.length > 0 ? parsedRows.length : ''} {parsedRows.length === 1 ? 'contato' : 'contatos'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
