...
export function summarizeNode(node: BuilderNode): string | null {
  ...
  switch (node.node_type) {
    ...
    case "send_list": {
      ...
      if (text.length > 0) {
        return rowCount > 0
          ? `${truncate(text, 50)} · ${rowCount} op${rowCount === 1 ? "ção" : "ções"}`
          : truncate(text);
      }
      return rowCount > 0
        ? `${rowCount} op${rowCount === 1 ? "ção" : "ções"} em ${sections.length} se${sections.length === 1 ? "ção" : "ções"}`
        : null;
    }
    case "send_media": {
      const mediaType =
    ...
    case "set_tag": {
      ...
      return tagId ? `${mode} tag ${tagId.slice(0, 8)}…` : `${mode} tag (nenhuma escolhida)`;
    }
    case "handoff": {
