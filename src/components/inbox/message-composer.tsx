...
export function MessageComposer({
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `O arquivo tem ${(file.size / 1024 / 1024).toFixed(1)} MB — o limite para ${kind} é de ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            A sessão de 24 horas expirou. Use um modelo para reatar o contato.
          </p>
          <Button
            variant="ghost"
            size="sm"
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? "Somente leitura — observadores podem visualizar mas não responder"
                : sessionExpired
                  ? "Sessão expirada - use um modelo"
                  : "Escreva uma mensagem... (Shift+Enter para nova linha)"
            }
