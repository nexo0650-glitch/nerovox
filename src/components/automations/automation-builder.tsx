...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova Mensagem Recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "Primeira vez que este contato envia uma mensagem (também funciona para contatos adicionados manualmente)",
  },
...
function AgentSelect({
  ...
  return (
    <select
      ...
      className={SELECT_CLASS}
    >
      <option value="">Selecionar um atendente…</option>
      {members.map((m) => (
      ...
      {value && !selected && (
        <option value={value}>{value} (atendente desconhecido)</option>
      )}
    </select>
  )
}
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "condition": {
      ...
      const subj = (step.step_config.subject as string) ?? ""
      const subjLabel = subjectLabels[subj] || subj || "?"
      return `quando ${subjLabel}`
    }
    case "send_webhook":
