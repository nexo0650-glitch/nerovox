...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Administrador', hint: 'Gerenciar membros + tudo' },
  { value: 'agent', label: 'Atendente', hint: 'Usar recursos; sem configurações' },
  { value: 'viewer', label: 'Visualizador', hint: 'Acesso de leitura em todo o app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`Cargo de ${member.full_name || 'membro'} atualizado para ${ROLE_META[nextRole].label}`);
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      toast.success(`${removingMember.full_name || 'Membro'} removido(a)`);
      setMembers((prev) =>
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        ...
        description="Pessoas com acesso a esta conta. Os cargos controlam o que cada colega de equipe pode fazer."
        action={
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              A URL original do convite é exibida apenas uma vez na criação por
              segurança — para compartilhar novamente, revogue o convite abaixo e
              crie um novo.
            </p>
          ) : null}
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              da conta? Ele será desconectado desta conta e receberá uma
              conta pessoal nova no seu próximo acesso. O login dele não
              será excluído.
            </DialogDescription>
          </DialogHeader>
