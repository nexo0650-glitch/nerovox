...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(`A identificação deve ter no máximo ${MAX_LABEL_LEN} caracteres`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta NeroVox',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'nossa conta NeroVox';
    const message = `Participe de ${accountName} no NeroVox usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
        ...
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Compartilhe este link com seu novo colega de equipe. Ele poderá se cadastrar
                (ou fazer login) e entrar na conta como{' '}
                <span className="font-medium text-muted-foreground">
                  {result.role === 'admin' ? 'Administrador' : result.role === 'agent' ? 'Atendente' : 'Visualizador'}
                </span>
                . O link é válido por{' '}
                <span className="font-medium text-muted-foreground">
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nós nunca armazenamos o link original — assim que você fechar esta janela,
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Gere um link de convite de uso único. Compartilhe-o via WhatsApp,
                Slack ou qualquer canal de sua preferência — sem necessidade de serviço de e-mail.
              </DialogDescription>
            </DialogHeader>
