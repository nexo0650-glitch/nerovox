      ...
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Defina campos adicionais para os contatos (ex: CEP, origem do lead). Eles
            aparecerão em todos os contatos e na ação de automação "Atualizar Campo do Contato".
          </DialogDescription>
        </DialogHeader>
        <CustomFieldsPanel />
      </DialogContent>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(`Um campo com o nome "${name}" já existe.`);
      return;
    }
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `Excluir "${field.field_name}"? Isso também removerá o valor armazenado desse campo em todos os contatos. Esta ação não pode ser desfeita.`
      )
    ) {
      return;
    }
    ...
    toast.success(`Excluído "${field.field_name}".`);
    await fetchFields();
  }
